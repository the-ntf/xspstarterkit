...
/**
 * @author padraic.edwards@ie.ibm.com
 *
 */
    ...
    private ResourceBundle getResourceBundle(String bundleSuffix, final Locale locale) {
        ...
        ResourceBundle bundle = AccessController.doPrivileged(new PrivilegedAction<ResourceBundle>() {
            public ResourceBundle run() {
                ...
                // Distinct from the Core runtime, ClassLoader is used in this instance
                // instead of a wrapped counterpart because the plugins which declare
                // this class provide both *.properties and *_en.properties resources when built
                // hence making the need for the wrapped ClassLoader redundant.
                ClassLoader classLoader = classInPackage.getClassLoader();
                if( null == classLoader ){
                    // This conditional ensures that classLoader is never null which would
                    // throw a NullPointerException for ResourceBundle.getBundle otherwise.
                    classLoader = this.getClass().getClassLoader();
                }
                try {
                    return ResourceBundle.getBundle( bundlePackage, locale, classLoader );
                }
