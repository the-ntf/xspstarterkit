...
public class AbstractDataView extends UIDataSourceIterator {
    ...
    /**
     * The default id for the inner auto-generated RowComponent control,
     * is added as a container for the row contents in the data-processing 
    ...
    private transient PhaseId processFacetsForPhase; // part of the workaround for SPR#MKEE8BFEDR
    
    private static final String[] CONTAINER_FACET_NAMES = {
            FACET_HEADER,
            FACET_PAGERTOP, 
            FACET_PAGERTOPLEFT, 
            FACET_PAGERTOPRIGHT, 
            FACET_PAGERBOTTOM, 
            FACET_PAGERBOTTOMLEFT, 
            FACET_PAGERBOTTOMRIGHT,
            FACET_FOOTER,
            FACET_NOROWS,
    };

    /**
     * Note, the facet names based on {@link #FACET_EXTRA_N} are also row-based facets.
     */
    ...
    public boolean isRowRefresh(FacesContext context) {
        if(ENABLE_PARTIAL_REFRESH_ROW) {
            ...
            if(/*ExtLibCompUtil.isXPages852() &&*/ XSPContext.getXSPContext(context).getUserAgent().isIE(0,8)) {
                return false;
            }
            return true;
        }
     ...
     * @param name the pageName to set
     */
    ...
    public void restoreState(FacesContext context, Object state) {
        ...
        summaryColumn = (SummaryColumn)StateHolderUtil.restoreObjectState(context, this, values[2]);
    ...
    public void buildContents(FacesContext context, FacesComponentBuilder builder) throws FacesException {
        // TODO update the UIDataView to add header/footer facets and allow prevent outerDiv/table
        ...
        // TODO this ID is not sufficiently unique - should prefix it with the control id or allow it to auto-generate
        // an ID or something. As it is, if you have 2 sibling xe:dataView or xe:forumViews on a page,
        ...
        // the row facets are repeated for each row (they are decoded / re-rendered for each row)
    ...
    /* (non-Javadoc)
     * @see com.ibm.xsp.component.UIDataIterator#restoreValueHolderState(javax.faces.context.FacesContext, javax.faces.component.UIComponent)
     */
    ...
    /* (non-Javadoc)
     * @see com.ibm.xsp.component.UIDataIterator#saveValueHolderState(javax.faces.context.FacesContext, javax.faces.component.UIComponent)
     */
    @SuppressWarnings("unchecked")//$NON-NLS-1$
    @Override
    ...
    /* (non-Javadoc)
     * @see com.ibm.xsp.extlib.component.data.UIDataSourceIterator#processDecodes(javax.faces.context.FacesContext)
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see com.ibm.xsp.extlib.component.data.UIDataSourceIterator#processUpdates(javax.faces.context.FacesContext)
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see com.ibm.xsp.extlib.component.data.UIDataSourceIterator#processValidators(javax.faces.context.FacesContext)
     */
    @Override
            ...
            try{
                ...
                if( getFacetCount() > 0 ){
                    ...
                    for (UIComponent facet: TypedUtil.getFacets(this).values()) {
                        ...
                        if( phaseOrdinal == PhaseId.APPLY_REQUEST_VALUES.getOrdinal() ){
                            facet.processDecodes(context);
                            continue;
                        }
                        if( phaseOrdinal == PhaseId.UPDATE_MODEL_VALUES.getOrdinal() ){
                            facet.processUpdates(context);
                            continue;
                        }
                        if( phaseOrdinal == PhaseId.PROCESS_VALIDATIONS.getOrdinal() ){
