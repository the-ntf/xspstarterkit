    ...
    public class _EntryMetaData extends EntryMetaData {
        ...
        protected _EntryMetaData(IPickerOptions options) throws NotesException {
            ...
            if( (valueIndex = findSortColumnIndex(vc))<0) {
                throw new FacesExceptionEx(null,com.ibm.xsp.extlib.domino.ResourceHandler.getSpecialAudienceString("DominoViewPickerData.Cannotfindavaluecolumnintheview0"),getView().getName()); // $NLX-DominoViewPickerData.Cannotfindavaluecolumnintheview0-1$
            }
            ...
            if(StringUtil.isNotEmpty(labelName)) {
                if( (labelIndex = findColumnIndex(vc, labelName))<0) {
                    throw new FacesExceptionEx(null,com.ibm.xsp.extlib.domino.ResourceHandler.getSpecialAudienceString("DominoViewPickerData.Cannotfindlabelcolumn0"),labelName); // $NLX-DominoViewPickerData.Cannotfindlabelcolumn0-1$
                }
            } else {
                labelIndex = -1;
            }
                ...
                try {
                    ...
                    if( ExtlibDominoLogger.DOMINO.isWarnEnabled() ){
                        ExtlibDominoLogger.DOMINO.warnp(this, "openView", ex, //$NON-NLS-1$ 
                                StringUtil.format(com.ibm.xsp.extlib.domino.ResourceHandler.getLoggingString("warn.DominoViewPickerData_ValuePickerNotSearchable_UnsortableColumn"),getViewName(),labelName)); // $NLW-DominoViewPickerData_ValuePickerNotSearchable_UnsortableColumn-1$
                    }
                }
            }
            return view;
