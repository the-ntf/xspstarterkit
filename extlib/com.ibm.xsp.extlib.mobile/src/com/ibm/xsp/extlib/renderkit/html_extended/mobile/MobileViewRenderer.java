...
/**
 * The MobileViewRenderer is a DataViewRenderer specifically tailored for displaying
 * a DataView control on mobile devices. 
 * @author jpierma
 *
 */
...
//    public static final int PROP_CATEGORYROWSCRIPT      = 141;
    ...
    protected Object getProperty(int prop) {
        switch(prop) {
            case PROP_CONTAINERTAG:         return "ul"; // $NON-NLS-1$
            case PROP_CONTAINERDOJOTYPE:    return "dojox.mobile.EdgeToEdgeList"; // $NON-NLS-1$
            case PROP_CONTAINERSTYLECLASS:  return "mblDataView"; // $NON-NLS-1$

            //case PROP_HEADERDOJOTYPE:       return "dojox.mobile.ListItem"; // $NON-NLS-1$
            //case PROP_HEADERCLASS:          return "mblVariableHeight"; // $NON-NLS-1$
            case PROP_HEADERSTYLE:          return "height: 2em"; // $NON-NLS-1$
            case PROP_HEADERCLASS:          return "mblVariableHeight mblListItem"; // $NON-NLS-1$
            //case PROP_FOOTERDOJOTYPE:       return "dojox.mobile.ListItem"; // $NON-NLS-1$
            //case PROP_FOOTERCLASS:          return "mblVariableHeight"; // $NON-NLS-1$
            
            case PROP_FOOTERCLASS:          return "mblVariableHeight mblListItem mblFooter"; // $NON-NLS-1$
            case PROP_FOOTER_PAGER_AREA_TAG: return "li"; // $NON-NLS-1$
            case PROP_LISTTAG:              return "div"; // $NON-NLS-1$
            case PROP_ITEMDIVSTYLECLASS:    return "mblListItemWrapper"; // $NON-NLS-1$
            case PROP_CATEGORYROWSTYLECLASS: return "mblCategoryRow"; // $NON-NLS-1$
            case PROP_DATAROWSTYLECLASS:    return "mblDataRow"; // $NON-NLS-1$
            case PROP_DATAROWICONSTYLECLASS: return "mblDataRowIcon"; // $NON-NLS-1$
            case PROP_ITEMTAG:              return "li"; // $NON-NLS-1$
            case PROP_ITEMDOJOTYPE:         return "extlib.dijit.mobile.ListItem"; // $NON-NLS-1$
            case PROP_ITEMSTYLECLASS:       return "mblVariableHeight"; // $NON-NLS-1$
            case PROP_ITEMTRANSITION:       return "slide"; // $NON-NLS-1$
            case PROP_SUBHEADTAG:           return "li"; // $NON-NLS-1$
            case PROP_SUBHEADDOJOTYPE:      return ""; // $NON-NLS-1$
            case PROP_SUBHEADDIVSTYLE:      return ""; // $NON-NLS-1$
            case PROP_SUBHEADSTYLECLASS:    return "mblDataViewCategoryRow"; // $NON-NLS-1$
            case PROP_SUBHEADSTYLECLASS_STD:    return "mblEdgeToEdgeCategory"; // $NON-NLS-1$
            case PROP_SUMMARYCOLSTYLE:      return ""; // $NON-NLS-1$
            case PROP_SUMMARYCOLSTYLECLASS: return "mblDataViewTextTitle"; // $NON-NLS-1$
            case PROP_SUMMARYCOLTEXTSTYLECLASS: return "mblDataViewTextContent"; // $NON-NLS-1$
//            case PROP_CATEGORYROWSCRIPT:    return "XSP.accordionifyCategory";
            
            case PROP_FOOTERSTYLE:          return ""; // $NON-NLS-1$
            case PROP_FOOTERLEFTSTYLE:      return ""; // $NON-NLS-1$
            case PROP_FOOTERRIGHTSTYLE:     return ""; // $NON-NLS-1$
            
            case PROP_FOOTERLEFTCLASS:      return "mblFooterText mblLeft"; // $NON-NLS-1$
            case PROP_FOOTERRIGHTCLASS:     return "mblFooterText mblRight"; // $NON-NLS-1$
            case PROP_FOOTERMIDDLECLASS:    return "mblFooterText"; // $NON-NLS-1$
    ...
    protected void writeDataView(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {      
        ...
        startElement(w,tag,null,PROP_CONTAINERSTYLECLASS,PROP_CONTAINERDOJOTYPE);
    ...
    protected void writeRowStart(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
        // Surrounding <div> tag
        ...
        String id = tableId + NamingContainer.SEPARATOR_CHAR+UIDataView.ROW_ID;
        
        String divStyleClass = (String)getProperty(PROP_ITEMDIVSTYLECLASS);
        if(isCategoryRow) {
            divStyleClass = ExtLibUtil.concatStyleClasses(divStyleClass, (String)getProperty(PROP_CATEGORYROWSTYLECLASS));
            boolean expanded = isRowExpanded(context, c, viewDef);
            id += (expanded?SHRINK_DELIMITER:EXPAND_DELIMITER) + viewDef.rowPosition;
        }
        else {
            divStyleClass = ExtLibUtil.concatStyleClasses(divStyleClass, (String)getProperty(PROP_DATAROWSTYLECLASS));
            if(viewDef.hasIconColumn) {
                divStyleClass = ExtLibUtil.concatStyleClasses(divStyleClass, (String)getProperty(PROP_DATAROWICONSTYLECLASS));
            }
        }
        ...
        if(isCategoryRow) {
            ...
            String styleClass;
            if(!themeName.equalsIgnoreCase(ApplicationRenderer.IPHONE_THEME_NAME) &&
                    !themeName.equalsIgnoreCase(ApplicationRenderer.ANDROID_THEME_NAME) &&
                        !themeName.equalsIgnoreCase(ApplicationRenderer.BLACKBERRY_THEME_NAME)){
    ...
    protected void writeRowListAttributes(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
        String djType = (String)getProperty(PROP_ITEMDOJOTYPE);
        if(StringUtil.isNotEmpty(djType)) {
            w.writeAttribute("dojoType", djType, null); // $NON-NLS-1$
        }
    ...
    protected String getRowUrl(FacesContext context, AbstractDataView c, ViewDefinition viewDef) {
    ...
    protected String getPageUrl(FacesContext context, AbstractDataView c, ViewDefinition viewDef) {
    ...
    protected void writeRowEnd(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
        if ( isCategoryRow(context, c, viewDef) ) {
            String tag = (String)getProperty(PROP_SUBHEADTAG);
            w.endElement(tag);
        } else {
    ...
    protected void writeCategory(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
        ...
        if(categoryFacet==null) {
            ...
            String styleClass = ExtLibUtil.concatStyleClasses((String)getProperty(PROP_EXTRACOLSTYLECLASS), categoryColumn.getStyleClass());
            if(StringUtil.isNotEmpty(styleClass)) {
                w.writeAttribute("class",styleClass,null); // $NON-NLS-1$
            }
            String style = ExtLibUtil.concatStyles((String)getProperty(PROP_EXTRACOLSTYLE),categoryColumn.getStyle());
    ...
    protected void writeSummary(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
        ...
        if(viewDef.summaryFacet==null) {
            w.startElement("div",c); // $NON-NLS-1$
            String styleClass = ExtLibUtil.concatStyleClasses((String)getProperty(PROP_SUMMARYCOLSTYLECLASS), viewDef.summaryColumn.getStyleClass());
            if(StringUtil.isNotEmpty(styleClass)) {
                w.writeAttribute("class",styleClass,null); // $NON-NLS-1$
            }
            String style = ExtLibUtil.concatStyles((String)getProperty(PROP_SUMMARYCOLSTYLE),viewDef.summaryColumn.getStyle());
            if(StringUtil.isNotEmpty(style)) {
                w.writeAttribute("style",style,null); // $NON-NLS-1$
            }
    ...
    protected void writeExtraColumn(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef, ExtraColumn col, int colIdx) throws IOException {
        ...
        if(facet!=null) {
            ...
            String styleClass = ExtLibUtil.concatStyleClasses((String)getProperty(PROP_EXTRACOLSTYLECLASS), col.getStyleClass());
            if(StringUtil.isNotEmpty(styleClass)) {
                w.writeAttribute("class",styleClass,null); // $NON-NLS-1$
            }
            String style = ExtLibUtil.concatStyles((String)getProperty(PROP_EXTRACOLSTYLE),col.getStyle());
            if(StringUtil.isNotEmpty(style)) {
                w.writeAttribute("style",style,null); // $NON-NLS-1$
            }
    ...
    protected String getIconColumnUrl(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
        if(viewDef.iconFacet!=null) //we are using an icon facet, this is handled later in writeStandardRow
            return null;
