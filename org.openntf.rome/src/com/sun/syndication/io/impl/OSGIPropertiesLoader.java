 ...
 * <P>
 * The master properties file has to be in a distinct location than the extra properties files. First the master properties file is loaded,
 * then all the extra properties files in their order of appearance in the classpath.
 * <P>
 * Current use cases (plugin manager for parsers/converters/generators for feeds and modules and date formats) assume properties are list of
 * tokens, that why the only method to get property values is the getTokenizedProperty().
 * <p>
 * 
...
public class OSGIPropertiesLoader {
	 ...
	 * 
	 */
	public static OSGIPropertiesLoader getPropertiesLoader() {
		synchronized (OSGIPropertiesLoader.class) {
			OSGIPropertiesLoader loader = (OSGIPropertiesLoader) clMap.get(Thread.currentThread().getContextClassLoader());
			if (loader == null) {
				try {
					loader = new OSGIPropertiesLoader(MASTER_PLUGIN_FILE, EXTRA_PLUGIN_FILE);
	 ...
	 * 
	 */
	private OSGIPropertiesLoader(String masterFileLocation, String extraFileLocation) throws IOException {
		List propertiesList = new ArrayList();

		try {
			InputStream is = getResource(masterFileLocation);
		...
		try {
			...
			IOException ex = new IOException("could not load ROME extensions plugins file [" + masterFileLocation.toString() + "], "
					+ ioex.getMessage());
			ex.setStackTrace(ioex.getStackTrace());
			throw ex;
		}
	...
	/**
	 * Returns an array of tokenized values stored under a property key in all properties files. If the master file has this property its
	 * tokens will be the first ones in the array.
	 * <p>
	 * 
	 * @param key
	 *            property key to retrieve values
	 * @param separator
	 *            String with all separator characters to tokenize from the values in all properties files.
	...
	/**
	 * Returns an array of values stored under a property key in all properties files. If the master file has this property it will be the
